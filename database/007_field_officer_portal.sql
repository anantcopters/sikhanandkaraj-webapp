...
CREATE INDEX IF NOT EXISTS idx_field_officer_login_otps_officer ON field_officer_login_otps (
    field_officer_id,
    created_at DESC
);

CREATE UNIQUE INDEX IF NOT EXISTS uq_field_officer_login_otps_pending ON field_officer_login_otps (field_officer_id)
WHERE
    status = 'PENDING';
...
FROM
    users u
    INNER JOIN member_family_details family ON family.user_id = u.id
    AND family.field_officer_id IS NOT NULL
    LEFT JOIN member_basic_details basic ON basic.user_id = u.id
    LEFT JOIN master_states state ON state.id = basic.state_id
    LEFT JOIN master_cities city ON city.id = basic.city_id
    LEFT JOIN LATERAL (
        SELECT REGEXP_REPLACE(
                contact.normalized_value, '^\+91', ''
            ) AS mobile_number
